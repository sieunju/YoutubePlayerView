...
>>>>>>> 282ef1a0ac0780a992d3f096db13df5d348e68b4
...
<<<<<<< HEAD
	...
	private var thumbnailType: ThumbnailType = ThumbnailType.AUTO // 썸네일 타입
    @IdRes
    private var thumbnailHardViewId : Int = -1
...
<<<<<<< HEAD
						...
						thumbnailType = ThumbnailType.values()[attr.getInt(R.styleable.YoutubePlayerView_youtube_thumb_type, 0)]
...
//            R.id.clThumbNail -> startVideo()
	...
	private fun initView() {
		...
		// Autu 인경우
		if (thumbnailType == ThumbnailType.AUTO) {
			if (container is ViewGroup) {
				thumbnailView = YoutubeThumbnailView(context).run {
					this.setOnClickListener { startVideo() }
					return@run this
				}
				(container as ViewGroup).addView(thumbnailView)
			}
		} else {
